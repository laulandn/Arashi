...
void	VADoFrame()
{
	...
	VAErasePixels(			VA.numpix[erasedbuffer],	VA.pix[erasedbuffer],erasedbuffer);
	VAEraseSpots(			VA.numspots[erasedbuffer],	VA.spots[erasedbuffer],erasedbuffer);

	VAPlotSpots(VA.numspots[VA.curbuffer],
				VA.spots[VA.curbuffer],
				VA.spotcolors[VA.curbuffer],VA.curbuffer);
	...
	VAPlotPixels(VA.numpix[VA.curbuffer],
				VA.pix[VA.curbuffer],
				VA.pixcolors[VA.curbuffer],VA.curbuffer);
#ifdef ORIG_ARASHI
	SwapMMUMode(&mode);
#endif
...
GDHandle	GDev;
{
	...
	{	VASpec[i]		= (void *) NewPtr(sizeof(ColorSpec)	* 256);
		...
		VA.pixx[i]		= (void *) NewPtr(sizeof(short)		* MAXSAVEDPIX);
		VA.pixy[i]		= (void *) NewPtr(sizeof(short)		* MAXSAVEDPIX);
		VA.spotcolors[i]= (void *) NewPtr(sizeof(int)		* MAXSAVEDPIX);
		VA.spots[i]		= (void *) NewPtr(sizeof(Ptr)		* MAXSAVEDPIX);
		VA.spotsx[i]		= (void *) NewPtr(sizeof(short)		* MAXSAVEDPIX);
		VA.spotsy[i]		= (void *) NewPtr(sizeof(short)		* MAXSAVEDPIX);
		VA.numlines[i]=0;
		VA.numpix[i]=0;
		VA.numspots[i]=0;
	}
