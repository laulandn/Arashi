/*/
     ...
     Last modification: Thursday, August 31, 1995, 1:28
...
void GameEvent(void);
void ZeroScore(void);
void WriteRecordedPlay(void);
void WritePlayRecord(void);
void UnloadPlayRecord(void);
void LoadPlayRecord(void);
void StartPlayback(void);
void StartPlayRecord(void);
void StartNormalPlay(void);
void AddLife(void);
void RemoveLife(void);
void STInitialize(void);
void STAllocate(void);
void STUpdate(void);
void STLoadLevel(void);
void STFlyOutLoop(void);
int GamePeekEvent(void);
void GamePause(int cleanup);
void DrawField(void);
void RedrawField(void);
void RedrawSpikes(void);
void DoInits(void);
void InitSoundKit(void);
int DoLevelSelect(int level);
...
//	StripAllPictComments();
	...
	if(PlayOptions->blankUnused)
		BackdropWind = NewWindow(0,&((*LMGetGrayRgn())->rgnBBox),&zero,-1,plainDBox,0,0,0);
	else
